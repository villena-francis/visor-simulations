...
in ``results/{stage}_{coverage}x_{length}_{replicate}_LASeR``.

> The config parameters "replicates", "coverage" and lenght determine the number of 
...
`results/normal_{coverage}x_{length}_LASeR`, where the coverage matches that of the 
...
stored in `results/{stage}_{coverage}x_{length}_{replicate}_{caller}`.
